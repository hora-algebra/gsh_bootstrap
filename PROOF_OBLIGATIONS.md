...
| N-A4FULL-002 | Decide step [3] of `A4-FULL-01`: that the mod-2 counts `N[g,p]` are reconstructible from the certified height-1 features for **every** word, not merely for words of length ≤ 4 plus 4,000 random ones. This is the single remaining gap keeping `A4-FULL-01`, `A4-ALLLANG-01`, `ORD12-ALL-01` and `FRONTIER-ORD20-01` at `EMPIRICAL`, i.e. keeping order 12 open and the frontier at `A_4` rather than `F_20`. Step [4] was closed on 2026-07-25 (`scripts/completeness_upgrade.py`). | Two pieces. (a) Each of the six per-mover GF(2) identities — `x_p + n_{p+e} = A_p + s·[p=0]` forward and its backward twin — is a relation among finitely many mod-2 accumulators, so it is decided by a product BFS over `(phase, prev letter, N[g,·], N'[gg,·], Σ_{h≠g} N'[hg,·], first-letter flag)`, roughly 40k states; the same shape as `prove_function` in `scripts/weis_l2_family.py`. (b) The backward features are features of the reversed word, which no forward automaton computes, so they must be carried as **transition profiles** of the reversed feature automaton (profile ∘ δ(·,g) on appending a letter). Only then is the full-rank check on the 6×6 system meaningful. | run `scripts/completeness_upgrade.py`; the acceptance test is that step [3] reports a decided verdict with a negative control that fires, exactly as steps [4], `WATOM-45-01` and `LAAB-04-01` now do | OPEN |
