...
| M-SFA-DECIDE-001 | Turn the `L2` rank measurement from a search into a decision. Hashiguchi 1988 makes relative star height `h_r(·, 𝒞)` computable for each FIXED finite base `𝒞`, and Kirsten 2011 (MFCS, LNCS 6907) gives the relative *inclusion* variant in triple exponential space, attributing the inclusion variants to Hashiguchi 1991 (TCS 91:85–100, doi:10.1016/0304-3975(91)90269-8). Since `r_SF(L) = min over finite 𝒞 ⊆ SF of h_r(L,𝒞)` (`notes/sf_labeled_automata.md` §7), the question "is there a rank-1 SF-automaton for `L2` over *this specified* finite label set" is decidable rather than a search. Fix the base actually used by `WEIS_L2_DIAGONAL_GRAPH` (the letters plus `a*`) and decide `h_r(L2, 𝒞)` for it. **This would still not be a gsh lower bound** (README research rule 1): the minimum over all of `SF` remains an infinite search, so a verdict of 2 is a statement about that base only. | either a decided value of `h_r(L2, 𝒞)` for the stated finite `𝒞`, with the algorithm implemented and its soundness argued, or an explicit `BLOCKED` recording which step of Hashiguchi's algorithm is not implementable at this scale | OPEN |
