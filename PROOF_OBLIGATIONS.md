...
| M-SFA-PRIOR-001 | Prior-art search for star height measured over star-free (rather than letter) labels. **Done 2026-07-25; findings in `notes/sf_labeled_automata.md` §7 and §2.1a, ledger `SFA-EGGAN-01`.** `SFA-EGGAN-01` is confirmed NOT new: the mechanism is Sakarovitch, "Automata and rational expressions" (arXiv:1502.03573) §3.6, eqs (3.24)–(3.26) + Property 3.12 + Prop. 3.13 (Lombardy–Sakarovitch 2003) + Thm. 3.11 (Eggan 1963), which already indexes generalised automata by the star height of their labels; that source's `h` is the restricted star height, so only the instantiation to generalized star height with star-free labels is added here. Also established: the bound is an equality over elimination orders. No automaton-side treatment of *generalized* star height found in Sakarovitch, Gruber (arXiv:1111.5357), Bourne's NBSAN slides, or *Star Height via Games* (arXiv:1708.03603). Residual, still `REVIEW`: (i) Hashiguchi's *relative star height* (Inf. Comput. 78(2):124–169, 1988) and Kirsten's *Star Height Substitutions* habilitation — structurally the same move as `r_SF`, exact definitions not retrieved (paywalled); (ii) PST 1992 itself not fetched (HAL bot filter), so unchecked for an automaton formulation. A five-item fetch list with exact DOIs, and a `SPECULATIVE` lead on the relative-star-height definition (the base class may be required to be *finite*, which would make `r_SF` related but not an instance), are in `notes/sf_labeled_automata.md` §7. | for the residual: the two definitions transcribed with page numbers, and a verdict on whether either subsumes `r_SF`; recorded in `notes/sf_labeled_automata.md` §7 | REVIEW |
