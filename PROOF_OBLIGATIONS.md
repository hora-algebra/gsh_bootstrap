...
| N-C7C3-001 | `HeightOneForGroup (C_7 ⋊ C_3)` — smallest odd-order group outside the PST class (order 21), and the minimal instance of Bourne's failed `A ⋊ C_3` step (`SMALL-C3-FAIL`). **Progress 2026-07-25 (`C7C3-FULL-01`, `RESULTS.md` §5.14)**: the `A4-FULL-01` multi-mover mechanism transposes to the **full 21-letter alphabet** — which by `FULL-ALPH-RED-01` is the whole obligation — with all 288 cut patterns aperiodic, GF(7) rank 6/6, and exact reconstruction of the identity fibre on all 204205 words of length ≤ 4. The phase group is `Z/3`, of prime order, so the `Z/4` obstruction of `F20-FULL-OBS-01` does not arise. | **Narrowed twice 2026-07-25**: the arithmetic half is now `PROVED` (`C7C3-IDENT-01`, `RESULTS.md` §5.15) — the identity fibre is a Boolean combination of the residue languages of 57 cut counts whose token languages are certified star-free, for every word and with no length bound. What is missing is exactly one thing: a **star-free expression** for each token language. Nothing else is needed, because with `V` the language of words that never cut, `T = ¬V ∩ ¬(¬V · Σ⁺)` and the residue language is `cat(T_first, T^h, star(T^7), V)`, so all remaining operations are Boolean, concatenation and a single star | (1) write the star-free expression for `V` for each of the 57 patterns and verify it by exact DFA equivalence against the directly built token automaton, then assemble with the running-sum tree and check the height symbolically; the live-state analysis says `V` is a *local* condition on consecutive movers (from a live phase `r ≠ q` exactly one mover class avoids `q`, so classes alternate) together with a star-free condition on the first mover, which is the only non-local ingredient; then compile per atom and prove equivalence with `equivalence_counterexample` of `scripts/weis_l2_full_gsh1.py`. Note the assembled expression need NOT be compiled: its correctness follows from `C7C3-IDENT-01` algebraically; (2) if (1) fails, fall back to the original route: enumerate the transitive actions (7 points, 3 cosets of `C_7`, 21 points) and test the anchor criterion on each, noting that `C_7 ⋊ C_3` has no subgroup of index 2 so parity-based fiber separation is unavailable | OPEN |
