...
1. **`N-FIB-PRIOR-001`（文献調査）を先にやる。** conjecture の一般形が既知か、
   反例が知られているかを確認しないまま時間を使わない。特に "wreath product principle"、
   "sequential function", "star height" の組み合わせ、および Straubing 1979/1985、
   Pin の *Varieties of Formal Languages* の該当章。
2. その上で conjecture の**最小の未知の場合**、すなわち state group `C_4`、
   ...
   **その expression を alphabet に依存しない形に書き直せるか**が具体的な作業単位に
   ...
   実行することになる。
3. 逆向きの検証として、conjecture の state group を **non-abelian** にすると偽になる
   はずである（そうでなければ `A_5` まで一挙に落ちてしまい、GSH-STATUS-01 と整合しない）。
   反例を探すのは conjecture の scope を正しく切るために有用で、かつ安い。
