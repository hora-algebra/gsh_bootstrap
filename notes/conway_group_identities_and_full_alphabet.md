...
- **証拠状態の注意**: 本ノートは `A4-FULL-01` を昇格させない。そこから導く $A_4$-言語全体の主張も、元の計算証拠と閉性定理の監査状態を継承する。**2026-07-25 の完全性監査により `A4-FULL-01` は `EMPIRICAL` に降格した**（特徴量の復元検証が長さ 4 まで＋ランダム 4,000 語、最終合成がランダム 2 万語のみ）。本ノートが以前ここに書いていた `COMPUTED` は撤回する（`RETRACTIONS.md` 第1項）。標本は反証はできても立証にはならないので、§4 の $A_4$ に関する帰結はすべて `EMPIRICAL` を継承する。
