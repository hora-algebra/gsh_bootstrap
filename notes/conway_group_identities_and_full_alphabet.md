...
- **証拠状態の注意**: 本ノートは `A4-FULL-01` を昇格させない。そこから導く $A_4$-言語全体の主張も、元の計算証拠と閉性定理の監査状態を継承する。**2026-07-25 の完全性監査により `A4-FULL-01` は `EMPIRICAL` に降格した**（特徴量の復元検証が長さ 4 まで＋ランダム 4,000 語、最終合成がstep [4] は網羅的に決定済みで、残るのは step [3] のみ）。本ノートが以前ここに書いていた `COMPUTED` は撤回する（`RETRACTIONS.md` 第1項）。標本は反証はできても立証にはならないので、§4 の $A_4$ に関する帰結はすべて `EMPIRICAL` を継承する。
...
- `A4-FULL-01`: 現在 `EMPIRICAL`（2026-07-25 に `COMPUTED` から降格。以前の記述は撤回）。
- 命題 3.1: PST の閉性仮説を精査したうえでの数学的還元。
- 「全 $A_4$-認識言語」の結論（台帳 `A4-ALLLANG-01`）: 上二つを合成し、弱い方を継ぐので `EMPIRICAL`。
  （このノートは以前ここを `UNREVIEWED` と書いていたが、同一ノート内で status が二重化するので撤回する。）
...
2. ~~**ledger**: `A4-ALLLANG-01` を追加する。~~ **完了**。ただし当初の想定と違い、
   この行は `EMPIRICAL` である（`A4-FULL-01` の降格を継承）。残作業は `N-A4FULL-002`。
