...
**限界.** この定理は4文字分解を構成しない。とくに定理Hが選ぶphase-preserving座標が補群で、
補題Gが要求する生成座標が別にある可能性を排除しない。生成する31 orbitについても
`gsh(T_S)≤1` は未証明である。従って `HeightOneForGroup F_20` は依然OPEN。
...
`|Δ_j|=4` である。定理Iにより、まずphase-preserving座標が (a) 31個の生成orbitの一つ、
(b) 唯一の補群orbit、のどちらかで場合分けできる。各代表について
`K∩(1×F_20)=1` の有限判定で候補を検証し、成功した場合だけ、その4文字 sub-alphabet に対する
height-one expressionを探す。異なる群を座標に許す変種には定理Hをそのまま適用できないので、
§10 の副問いとして分離する。
