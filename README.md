...
- **⚠️ 要監査**: 既知定理で被覆されるとGAPが全数判定したが、その判定をrepo内の独立checkerがまだ再検査していない（`COVER-LE60-01` は `UNREVIEWED`）。
- **× 未知**: full solutionは未証明。部分成果がある場合は根拠欄に明記する。

内訳は、**✅ Lean形式化完了 0群、⭕️ 証明完了 41群、⚠️ 要監査 137群、× 未知 32群**。GAP判定では178群が既知定理またはrepo内証明で被覆され、そのうち位数31以下の41群だけが現在の台帳水準まで監査済みである。A4独自証明を入れる前の系列では40群が残るが、`A_4` とそのsubdirect帰結7群を `A4-ALLLANG-01` / `SUBDIRECT-RED-01` が回収し、現在の未知候補は32群（うちmonolithicで直接攻撃が必要なのは24群）。これは高さ2以上の下界ではない。
...
| 34 | `SmallGroup(34, 1) ≅ D34` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 36 | `SmallGroup(36, 4) ≅ D36`, `SmallGroup(36, 10) ≅ S3 x S3`, `SmallGroup(36, 12) ≅ C6 x S3`, `SmallGroup(36, 13) ≅ C2 x ((C3 x C3) : C2)` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 36 | `SmallGroup(36, 1) ≅ C9 : C4` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 36 | `SmallGroup(36, 3) ≅ (C2 x C2) : C9`, `SmallGroup(36, 6) ≅ C3 x (C3 : C4)`, `SmallGroup(36, 7) ≅ (C3 x C3) : C4`, `SmallGroup(36, 11) ≅ C3 x A4` | subdirect還元 | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 36 | `SmallGroup(36, 9) ≅ (C3 x C3) : C4` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 38 | `SmallGroup(38, 1) ≅ D38` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 39 | `SmallGroup(39, 1) ≅ C13 : C3` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 40 | `SmallGroup(40, 10) ≅ C5 x D8`, `SmallGroup(40, 11) ≅ C5 x Q8` | nil₂ | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 40 | `SmallGroup(40, 5) ≅ C4 x D10`, `SmallGroup(40, 6) ≅ D40`, `SmallGroup(40, 8) ≅ (C10 x C2) : C2`, `SmallGroup(40, 13) ≅ C2 x C2 x D10` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 40 | `SmallGroup(40, 4) ≅ C5 : Q8` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 40 | `SmallGroup(40, 1) ≅ C5 : C8`, `SmallGroup(40, 7) ≅ C2 x (C5 : C4)` | subdirect還元 | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 40 | `SmallGroup(40, 3) ≅ C5 : C8`, `SmallGroup(40, 12) ≅ C2 x (C5 : C4)` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 42 | `SmallGroup(42, 3) ≅ C7 x S3`, `SmallGroup(42, 4) ≅ C3 x D14`, `SmallGroup(42, 5) ≅ D42` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 42 | `SmallGroup(42, 1) ≅ C7 : C6`, `SmallGroup(42, 2) ≅ C2 x (C7 : C3)` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 44 | `SmallGroup(44, 3) ≅ D44` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 44 | `SmallGroup(44, 1) ≅ C11 : C4` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 46 | `SmallGroup(46, 1) ≅ D46` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 48 | `SmallGroup(48, 21) ≅ C3 x ((C4 x C2) : C2)`, `SmallGroup(48, 22) ≅ C3 x (C4 : C4)`, `SmallGroup(48, 24) ≅ C3 x (C8 : C2)`, `SmallGroup(48, 45) ≅ C6 x D8`, `SmallGroup(48, 46) ≅ C6 x Q8`, `SmallGroup(48, 47) ≅ C3 x ((C4 x C2) : C2)` | nil₂ | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 48 | `SmallGroup(48, 4) ≅ C8 x S3`, `SmallGroup(48, 5) ≅ C24 : C2`, `SmallGroup(48, 6) ≅ C24 : C2`, `SmallGroup(48, 7) ≅ D48`, `SmallGroup(48, 14) ≅ (C12 x C2) : C2`, `SmallGroup(48, 25) ≅ C3 x D16`, `SmallGroup(48, 26) ≅ C3 x QD16`, `SmallGroup(48, 35) ≅ C2 x C4 x S3`, `SmallGroup(48, 36) ≅ C2 x D24`, `SmallGroup(48, 37) ≅ (C12 x C2) : C2`, `SmallGroup(48, 38) ≅ D8 x S3`, `SmallGroup(48, 43) ≅ C2 x ((C6 x C2) : C2)`, `SmallGroup(48, 51) ≅ C2 x C2 x C2 x S3` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 48 | `SmallGroup(48, 8) ≅ C3 : Q16` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 48 | `SmallGroup(48, 1) ≅ C3 : C16`, `SmallGroup(48, 9) ≅ C2 x (C3 : C8)`, `SmallGroup(48, 10) ≅ (C3 : C8) : C2`, `SmallGroup(48, 11) ≅ C4 x (C3 : C4)`, `SmallGroup(48, 12) ≅ (C3 : C4) : C4`, `SmallGroup(48, 13) ≅ C12 : C4`, `SmallGroup(48, 15) ≅ (C3 x D8) : C2`, `SmallGroup(48, 16) ≅ (C3 : Q8) : C2`, `SmallGroup(48, 17) ≅ (C3 x Q8) : C2`, `SmallGroup(48, 18) ≅ C3 : Q16`, `SmallGroup(48, 19) ≅ (C6 x C2) : C4`, `SmallGroup(48, 27) ≅ C3 x Q16`, `SmallGroup(48, 31) ≅ C4 x A4`, `SmallGroup(48, 34) ≅ C2 x (C3 : Q8)`, `SmallGroup(48, 39) ≅ (C4 x S3) : C2`, `SmallGroup(48, 40) ≅ Q8 x S3`, `SmallGroup(48, 41) ≅ (C4 x S3) : C2`, `SmallGroup(48, 42) ≅ C2 x C2 x (C3 : C4)`, `SmallGroup(48, 49) ≅ C2 x C2 x A4`, `SmallGroup(48, 50) ≅ (C2 x C2 x C2 x C2) : C3` | subdirect還元 | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
...
| 50 | `SmallGroup(50, 1) ≅ D50`, `SmallGroup(50, 3) ≅ C5 x D10`, `SmallGroup(50, 4) ≅ (C5 x C5) : C2` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 52 | `SmallGroup(52, 4) ≅ D52` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 52 | `SmallGroup(52, 1) ≅ C13 : C4` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 52 | `SmallGroup(52, 3) ≅ C13 : C4` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 54 | `SmallGroup(54, 10) ≅ C2 x ((C3 x C3) : C3)`, `SmallGroup(54, 11) ≅ C2 x (C9 : C3)` | nil₂ | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 54 | `SmallGroup(54, 1) ≅ D54`, `SmallGroup(54, 3) ≅ C3 x D18`, `SmallGroup(54, 4) ≅ C9 x S3`, `SmallGroup(54, 7) ≅ (C9 x C3) : C2`, `SmallGroup(54, 12) ≅ C3 x C3 x S3`, `SmallGroup(54, 13) ≅ C3 x ((C3 x C3) : C2)`, `SmallGroup(54, 14) ≅ (C3 x C3 x C3) : C2` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
...
| 56 | `SmallGroup(56, 9) ≅ C7 x D8`, `SmallGroup(56, 10) ≅ C7 x Q8` | nil₂ | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 56 | `SmallGroup(56, 4) ≅ C4 x D14`, `SmallGroup(56, 5) ≅ D56`, `SmallGroup(56, 7) ≅ (C14 x C2) : C2`, `SmallGroup(56, 12) ≅ C2 x C2 x D14` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 56 | `SmallGroup(56, 3) ≅ C7 : Q8` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 56 | `SmallGroup(56, 1) ≅ C7 : C8`, `SmallGroup(56, 6) ≅ C2 x (C7 : C4)` | subdirect還元 | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 56 | `SmallGroup(56, 11) ≅ (C2 x C2 x C2) : C7` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 57 | `SmallGroup(57, 1) ≅ C19 : C3` | 現行被覆外 | **× 未知** — 現行の被覆機構では未到達 |
| 58 | `SmallGroup(58, 1) ≅ D58` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 60 | `SmallGroup(60, 8) ≅ S3 x D10`, `SmallGroup(60, 10) ≅ C6 x D10`, `SmallGroup(60, 11) ≅ C10 x S3`, `SmallGroup(60, 12) ≅ D60` | A⋊E | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 60 | `SmallGroup(60, 3) ≅ C15 : C4` | div | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
| 60 | `SmallGroup(60, 1) ≅ C5 x (C3 : C4)`, `SmallGroup(60, 2) ≅ C3 x (C5 : C4)`, `SmallGroup(60, 9) ≅ C5 x A4` | subdirect還元 | **⚠️ 要監査** — `COVER-LE60-01` は `UNREVIEWED` |
...
**範囲と出典の読み方**: `CITED`, `COMPUTED`, `PROVED`, `EMPIRICAL`, `UNREVIEWED` は `CLAIMS_LEDGER.md` 冒頭の規範的な意味で用いる。とくに、群がPST定理の仮定に入るという計算は、それが参照する高さ定理そのものの証明ではない。監査対象は明記したPST定理と本リポジトリ内の還元であり、1992〜2026年の他文献は網羅調査していない。したがって `OPEN` はこの監査記録に相対的な状態であって、全出版物に関する断言ではない。**「現行被覆外」は決して下界ではない**。位数60の `A_5` は未解決だが、一部の生成射は `COMPUTED` である（§5.6〜5.7）。生成表・完全なID一覧・検査境界は `data/experiments/coverage_le60.tsv` と `notes/small_group_coverage_le60.md` にある。
...
| `scripts/research/small_group_pst_coverage.py` / `scripts/gap/coverage_le60.g` | 位数 ≤ 31 の独立監査と、位数 ≤ 60 のGAP被覆判定（上の一覧表の根拠）。 |
