...
| 逆 alphabetic morphism の閉包を自前化 | P2 | `h^{-1}` が gsh ≤ 1 を保つことを4段で自証。phase-character projectorにより、このschemeを`F_20`座標だけで組む限り最大座標alphabetは5文字以上。一方、16個のphase局在threshold座標ですべてを正確に5文字にする分解を構成したので、この下界は鋭い。identity erasure後、`F_20` 全体は4種類の具体的な生成4文字alphabetのidentity fibreに還元された。ただしその4件はまだ OPEN | ✅ `ALPH-RED-01` / `F20-ALPH4-RIGID-01` / `F20-ALPH5-01` PROVED；`N-F20-GAMMA4-001` OPEN |
