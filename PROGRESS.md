...
| 逆 alphabetic morphism の閉包を自前化 | P2 | `h^{-1}` が gsh ≤ 1 を保つことを4段で自証。`PST-CL-01`（CITED、hypotheses 未検証）への依存を切った。副産物で `F_20` の20文字が8文字、identity erasure後は7文字に落ちる。ただし旧cut機構は7文字でも最短長4のfeature collisionを持つ | ✅ `ALPH-RED-01` / `F20-ALPH8-01` PROVED；`F20-ALPH7-OBS-01` COMPUTED |
