...
| `C_7⋊C_3` 全21元アルファベット | P3 | `F_20` の障害診断（下記）から「phase 群が素数位数なら機構が動く」と予測し、非周期性 288/288・GF(7) 階数 6/6 が通った。その後**式も言語同値も出た**: 277,408 ノードの共有 DAG を組み、群の乗法だけから作った21状態の語問題オートマトンとの同値を積到達可能性で決定（標本なし・語長制限なし）、独立再実行で再現。**それでも「解けた」ではない** — 証明書チェッカーが共有 DAG を受け付けないので台帳は上がらない | ⚠️ `C7C3-HEIGHTONE-01` UNREVIEWED／`C7C3-FULL-01` EMPIRICAL（`C7C3-IDENT-01` の再構成部分は PROVED）。障害は `N-C7C3-EXPR-001` の1点 |
