...
| PARTIAL | `A₄` の明示的 `ZMod 3` 位相と `V₄` 座標をLeanで分離し、full identity fibreとの正確な交叉等式、位相定理を実際に消費する親定理、および各文字を読む直前の位相付き出現回数からの `V₄` 座標の全語復元を閉じた（`L-A4-PHASE-001`, `L-A4-COORD-001`）。残る本体は `HasHeightAtMost a4PhaseLabelCoordinateFiber 1` |
