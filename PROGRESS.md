...
- **Lean 側**: 可換有限群すべて、位数 ≤ 5 の全群、`S₃`、位数8の `D₄` と `Q₈`、位数10の `D₅`、位数12の `D₆` と `Dic₃` で
  ...
  自己ループ側と反転文字側の両方について、全語・両source・法3の全矢印剰余言語を
  ...
  高さ1まで閉じた（`L-D4-SELF-LOOP-SOURCE0-001`）。最後に source-one を別言語として構成せず、
  ...
  mod-five arrow fingerprint、phase-one/source-zeroの有限Boolean組立、self-loop residueも閉じ、
  `GSH.heightOne_fullIdentityFiber_D5` を経て `GSH.heightOne_D5` までexact quantifiersのまま証明した
  （`L-D5-FLIP-ARITH-001`、`L-D5-SELF-LOOP-SOURCE0-001`、`L-D5-001`）。
...
| PARTIAL | `A₄` の phase/`V₄` 分離と全語座標復元に加え、36個の phase-before 文字出現回数の偶奇特徴への正確な有限 Boolean 分解を閉じた（`L-A4-PHASE-001`, `L-A4-COORD-001`, `L-A4-PARITY-001`）。phase-zero 文字については、ノート §5.1 の全語恒等式 `N[h,q] = Z_q + F(q,h) (mod 2)` を閉じ（`L-A4-NONMOVER-001`）、base-cut 偶奇言語の全語 first-return 式 `O (X X)* V` / `V₀ ∪ O X (X X)* V` と条件付き高さ1合成までLean化した（`L-A4-BASE-CUT-PARITY-001`）。さらにXのtarget非依存性、cut数のphase trace化、具体的height-0候補式、内部の交互cycleの正確な `(Z* P₁ Z* P₂)*` / 対称denotation、および二つの非零状態の一文字 first-return 再帰式まで閉じた（`L-A4-CUT-COMPONENTS-001`）。Xについて残るのは、このcycle denotationと二つの終端分岐を再帰式へ結合し、全語同値を閉じる一つの橋渡し。その後に `O,V,V₀`、single-delay版、mover再構成が続く。4状態 point-return DFA 表現（`L-A4-POINT-001`）は正確な別表現だが、高さの穴自体は弱めない |
