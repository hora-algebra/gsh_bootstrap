...
| 逆 alphabetic morphism の閉包を自前化 | P2 | `h^{-1}` が gsh ≤ 1 を保つことを4段で自証。`F_20` の20文字は8文字、identity erasure後は7文字に落ちる。ただし旧cut機構は7文字でも衝突する。さらにphase-rigidityにより、このschemeを`F_20`座標だけで組む限り少なくとも1座標は4文字以上で、2/3文字還元は不可能。境界のfull-phase 4文字は625個中620個が生成し、自己同型で31生成orbit＋1補群orbitに縮約 | ✅ `ALPH-RED-01` / `F20-ALPH8-01` / `F20-PHASE-RIGID-01` / `F20-ALPH4-CLASS-01` PROVED；`F20-ALPH7-OBS-01` COMPUTED |
