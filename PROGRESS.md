...
| PARTIAL | `A₄` の phase/`V₄` 分離と全語座標復元に加え、36個の phase-before 文字出現回数の偶奇特徴への正確な有限 Boolean 分解を閉じた（`L-A4-PHASE-001`, `L-A4-COORD-001`, `L-A4-PARITY-001`）。phase-zero 文字については、ノート §5.1 の全語恒等式 `N[h,q] = Z_q + F(q,h) (mod 2)` もLeanで閉じた（`L-A4-NONMOVER-001`）。4状態 point-return DFA 表現（`L-A4-POINT-001`）は正確な別表現だが、高さの穴自体は弱めない。次は base/single-delay cut 偶奇言語の高さ1 block式 |
