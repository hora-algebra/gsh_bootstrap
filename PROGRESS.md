...
| PARTIAL | `A₄` の phase/`V₄` 分離と全語座標復元、36個の phase-before 偶奇特徴への有限 Boolean 分解、phase-zero文字の恒等式、base-cut偶奇言語の first-return 式までLean化済み。Xについてはtarget非依存性、height-0候補式、交互cycleと終端式の正確なdenotation、二つのfirst-return再帰式まで閉じた（`L-A4-CUT-COMPONENTS-001`）。regex側の整理は完了し、残るのは純言語同値 `branch1L = a4BaseCutFirstReturnFromZero 1` とstate 2版の2本。その後に `O,V,V₀`、single-delay版、mover再構成が続く。4状態 point-return DFA は正確な別表現だが高さの穴自体は弱めない |
