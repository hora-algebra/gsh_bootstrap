...
上限は基底 `A4-FULL-01` の水準で決まり、その全元版の再構成チェックは
有界長全数＋ランダム、すなわち有限標本にとどまる。標本は反証はできるが
立証はできないので、この行は A4 の主張の根拠にならない。独立な人間査読も全行で未了。

**まとめ（位数 ≤ 12）**: 位数 ≤ 12 は **A4 を除いて**「任意の群 G が認識する
...
従って Bourne の障壁のうち **Dic3 側は通過したが、A4 側は通過していない** —
§5–5.5 の counting 構成＋命題 3.1 は高さ 1 の構成を与えるものの、その検証は
有限標本にとどまり（`A4-FULL-01`、`EMPIRICAL`）、A4 は未解決のままである。
（2026-07-25 の完全性監査以前、ここには「両側とも通過した」と書かれていた。撤回。）
...
| 位数 | 群 | 備考 |
|---|---|---|
| 12 | **A4** | **最小の未解決非可換群**。高さ 1 の構成はあるが検証は有限標本（`A4-ALLLANG-01`, `EMPIRICAL`） |
| 20 | F20 = C5⋊C4（忠実） | A4 の次の未解決非可換群 |
...
**帰結（`FRONTIER-ORD20-01`、`COMPUTED`）**: **障壁は位数 12 のままである。**
A4 が PST クラス外であることは完全判定の結論だが、A4 を別経路で落としたという
主張（§5.5）は有限標本にとどまるので、障壁を越えたことにはならない。
（2026-07-25 の完全性監査以前、ここには「障壁は位数 12 から位数 20 に移った」と
書かれていた。撤回。F20 が最前線になるのは `N-A4FULL-002` が閉じた後である。）
...
3 段階の試行を経て**高さ 1 とする構成に到達した。ただし検証は完結していない**（`A4-FULL-01`、**`EMPIRICAL`**、2026-07-25 の完全性監査による降格）。以下の 4 ステップのうち完全なのは step 1 だけで、復元と合成は有限標本にとどまる。**この節の結論は主張の根拠にならない。**
...
**到達点（`EMPIRICAL`、2026-07-25 監査）: A4 の全 12 元アルファベット版について、高さ 1 の構成が得られ、
...
したがって `HeightOneForGroup A_4` は**本リポジトリでは未確立**であり、`FULL-ALPH-RED-01`（`PROVED`）が
要求する全元アルファベット版がここで止まっている。2 生成元版（§5、`A4-STD-01`/`A4-STD-02`）は
product reachability による完全証明がついているが、そこからは全元版は従わない。
...
§3 の完全判定で F20 = C5⋊C4（忠実作用、位数 20）が **A4 の次の未解決非可換群**と
確定した（`FRONTIER-ORD20-01`、`COMPUTED`。2026-07-25 の監査以前は「最小の未解決
非可換群」と書いていたが、A4 が未解決に戻ったので撤回）。その最初の一歩として、標準的な 2 生成元での
...
| `A_4` | 12 | `V_4` | MONOLITHIC（**未解決**、`A4-FULL-01` は `EMPIRICAL`） |
...
**2026-07-25 の完全性監査による訂正**: `A4-ALLLANG-01` は `EMPIRICAL` なので、
`C_2×A_4` は `FRONTIER-ORD20-01` のリストから*外れる*のではなく **`A_4` に合流する**。
独立な問題として残るのは `A_4`, `F_20`, `C_7⋊C_3`, `SL(2,3)`, `S_4` の 5 群である。
（この段落は以前「`C_2×A_4` はリストから外れ、残るのは 4 群、status は `A4-ALLLANG-01`
を継いで `COMPUTED`」と書いていた。`A_4` の解決を前提にしていたので撤回する。）
...
- **位数 ≤ 31 の非可換群 45 個のうち、既知の PST クラスに入らないのは
  ...
  `SMALL-NONAB-31-01`、`COMPUTED`）。最小の未解決対象は位数 12 の A4 であり、
  F20 = C5⋊C4 はその次に来る（`FRONTIER-ORD20-01`、`COMPUTED`）。**
  ...
  **A4 に合流する**。したがって独立な問題は `A4`(12), `F20`(20), `C7⋊C3`(21),
  `SL(2,3)`(24), `S4`(24) の **5 群**で、すべて monolithic である。同じ理由で
  `C2×S4` は `S4` に帰着する（これは A4 に依存しないので無条件）。
  副産物として、すべての双環群
  （したがってすべての一般化四元数群）が PST クラスに入ることを一律の埋め込みで
  示した（`DICM-EMB-01`、`PROVED`）。
- **位数 ≤ 12 は A4 を除いて記録された（`ORD12-ALL-01` は `EMPIRICAL`）。**
...
- **A4 word problem の全 12 元アルファベット版は、高さ 1 の構成を得たが
  検証は有限標本にとどまる（§5.5、`A4-FULL-01`、`EMPIRICAL`）。**
  全 A4-認識言語への還元は `PROVED`（命題 3.1、§3.5 追記）だが、
  標本は立証にならないので合成結論も `EMPIRICAL` を継承する。
  残作業は `PROOF_OBLIGATIONS.md` の `N-A4FULL-002`。
