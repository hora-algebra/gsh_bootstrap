...
- 既知の肯定的結果（高さ ≤ 1 が証明済みのクラス）:
  ...
  - L(u,k,n) で |u| ≤ 3（3 文字が相異なる場合）かつ n が平方因子なし
    （PST 1992, Theorem 7.5）
...
- **候補 1**: L(aab,0,4) = { w ∈ {a,b}* : (aab の散在部分語としての出現回数)
  ≡ 0 mod 4 }。**［2026-07-28 訂正］この候補選定は誤りだった。** 「|u| = 3 かつ
  法 4（平方因子あり）は PST が明示的に未解決としたパラメータ領域」と当初
  書いたが、一次資料を読むと PST 1992 の Theorem 7.4 が
  `h(L(a^i b a^j, k, n)) ≤ 1` を **i, j, k, n すべてについて**（squarefree 条件
  なし）述べており、`aab = a^2 b a^0` はその instance である。PST が未解決と
  したのは |u| = 3 で **3 文字が相異なる**場合の非 squarefree な n（Theorem 7.5
  が n の squarefree 性を仮定する）。以下の分解と検証は既知定理の独立な機械的
  再確認として有効だが、新規結果ではない（台帳 `LAAB-04-01`）。
...
なった。さらに 2026-07-27、§5.5 の旧サンプル箇所を有限状態の全探索へ
置き換えた `A4-FULL-FINITE-CORE-01` と、Schützenberger 1965 の Main Property
を用いる人間証明 `notes/a4_full_alphabet_exact.md` を追加した。敵対的査読は
具体的な論理的ギャップを認めず、全 17 negative controls も固定した。
従って `A4-FULL-01` と、これを全元アルファベット還元に合成した
`A4-ALLLANG-01` は **`PROVED`** である。ただし A4 前提そのものの Lean 証明
`GSH.heightOne_A4` は未実装である。

**まとめ（位数 ≤ 12）**: 各個別ケースについて「任意の群 G が認識する
任意の言語はgeneralized star-height ≤ 1」が次の検証水準で記録された —
...
**A4 は `PROVED`**（`A4-ALLLANG-01`）。総合行 `ORD12-ALL-01` は、五群分類・
被覆の独立監査と Lean 合成が未了なので、現時点では `UNREVIEWED` とする。
...
**帰結（`FRONTIER-ORD20-01`、`COMPUTED`、2026-07-27 更新）**:
**監査済み障壁は位数 20 である。** A4 の新しい全語証明が完成し、
`C_2×A_4` も subdirect 還元で従うため、F20 が最前線となった。
2026-07-25 の撤回は旧サンプル証拠に対して正しく、今回の結論は新しい
`A4-FULL-FINITE-CORE-01` と査読済み証明に基づく。
...
Z/3 phaseを動かす（multi-mover）ため §5 の分業構成が直接は使えない。
3 段階の試行を経て高さ 1 の構成に到達した。以下には 2026-07-25 時点の
旧サンプル経路も履歴として残すが、2026-07-27 に
`scripts/ci/completeness_upgrade.py` と
`notes/a4_full_alphabet_exact.md` が有限ギャップを修復した。現在の結論は
`A4-FULL-01`、**`PROVED`** である。
...
**旧到達点（`EMPIRICAL`、2026-07-25 監査）:** A4 の全 12 元アルファベット版について、高さ 1 の構成が得られ、
有限標本の範囲で反例が出ていない。** これは**決着ではない**。特徴量からの `N[g,p]` の復元は長さ 4 まで
（22,621 語）＋ランダム 4,000 語、`(P, N[g,p]) → 群要素` の合成はランダム 2 万語のみで、いずれも有限対象への
帰着と全数走査を経ていない。3 文字版 `{u,d,k}` には 384 状態積オートマトンによる全数証明があるので
（上記 `a4_full3.py`）、12 文字版に同じものを与えるのが昇格の道筋である。`weis_l2_family.py` の
`prove_function` 型の積 BFS が雛形になる。

**完全性修復（2026-07-27）:** `check_a4_full_token_factorization` は 279 パターン
の一意 cut 分解を 137,494 状態で、`check_a4_full_feature_equations` は 5,792
状態で、`check_a4_full_reconstruction` は全 8 movers・36,872 状態で、
step [4] は 12 状態で決定する。親 verdict
`A4-FULL-FINITE-CORE-01` は合計 238,742 状態、17/17 controls で
`COMPUTED`。Schützenberger の aperiodic-to-star-free Main Property、
...
§3 の完全判定と 2026-07-27 の A4 証明により、F20 = C5⋊C4
（忠実作用、位数 20）が **最小の未解決非可換群**となった
（`FRONTIER-ORD20-01`、`COMPUTED`）。その最初の一歩として、標準的な 2 生成元での
...
§5.16（`C7C3-FULL-01`）は再構成を長さ 4 までの網羅と sweep で確かめただけであり、
2026-07-25 の完全性監査で `EMPIRICAL` に降格した（当時の表記は `COMPUTED`）。
本節はその**算術部分を全語に対する定理に格上げ**する。ただし認証族は下で圧縮した
57 原子であり、§5.16 の 741 conditioned cuts そのものではないので、§5.16 の行が
遡って `COMPUTED` になるわけではない。
...
- **位数 ≤ 31 の非可換群 45 個のうち、既知の PST クラスに入らないのは
  ちょうど 6 群（A4, F20, C7⋊C3, SL(2,3), S4, C2×A4）である（§3、
  `SMALL-NONAB-31-01`、`COMPUTED`）。A4 と C2×A4 は 2026-07-27 に閉じ、
  最小の未解決対象は位数 20 の F20 = C5⋊C4 である
  ...
  **A4 に合流する**。A4 が解決した現在、独立な未解決問題は
  ...
  2026-08-04、二項直積部分は `GSH.heightOneForGroup_prod` として Lean 化され、
  PR #53 の `A₄` 定理を入力とする `GSH.heightOne_C2_prod_A4` も検査済みとなった
  （`LEAN-PROD-01` / `LEAN-A4PROD-01`）。これは `A₄` 証明の変更や新規性主張ではない。
  副産物として、すべての双環群
  （したがってすべての一般化四元数群）が PST クラスに入ることを一律の埋め込みで
  示した（`DICM-EMB-01`、`PROVED`）。
- **位数 ≤ 12 の各個別ケースは記録された（`ORD12-ALL-01` は `UNREVIEWED`）。**
  位数 < 12 と C12・C6×C2・Dih6 は
  `CITED`（PST 1992）、Dic3 は §3 の明示的埋め込み
  `Dic3 ↪ (C3×C4)⋊C2`（`CITED` + 群論計算 `PROVED`、機械検証
  `scripts/research/dic3_embedding.py`）。**A4 は `A4-ALLLANG-01` により
  `PROVED`**。Bourne (2017) の A4/Dic3 個別障壁は両側とも通過した。
  総合行が `UNREVIEWED` なのは五群分類・被覆の独立監査と Lean 化が未了だからである。
- **候補 2（A4 word problem、生成元 (123),(12)(34)）: generalized star-height 1。
  反例ではない（`A4-STD-01`、`COMPUTED`。384 状態積オートマトンの
  完全証明 `A4-STD-02` による）。** ただしこれは**この 2 生成元での
  word problem のみ**であり、`HeightOneForGroup A4` ではない。
...
- **候補 1（L(aab,0,4)）: generalized star-height 1。反例ではない。**
  単一剰余への分解＋繰り上がりビット分解で落ちる。**ただし新規結果ではない**:
  PST 1992 Theorem 7.4 が `L(a^i b a^j, k, n)` を全 i, j, k, n で覆っており
  （squarefree 条件なし）、`aab = a^2 b a^0` はその instance（§3 の訂正、
  台帳 `LAAB-04-01`）。ここでの寄与は独立な機械検証のみ。
...
- 一方 PST (1992) は「transfer lemma で Thomas の結果の簡単な証明が得られる」と
  ...
  `EMPIRICAL` / `UNREVIEWED` 部分である。PST の位数 12 未満定理は variety レベル
