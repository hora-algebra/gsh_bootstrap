...
/-!
# Star-free pieces for the `S₃` self-loop arrow language
...
namespace S3SelfLoopHeight

/-- A height-zero expression for the one-letter language `{label}`. -/
def labelR (label : S3Alphabet) : GRegex S3Alphabet := GRegex.atom label

/-- A height-zero expression for all constant words over `label`. -/
noncomputable def labelStarR (label : S3Alphabet) : GRegex S3Alphabet :=
  FiniteAlphabet.onlyWhere fun a => a = label

/-- The neutral letters other than the counted label. -/
noncomputable def neutralOtherR (label : S3Alphabet) : GRegex S3Alphabet :=
  FiniteAlphabet.atomWhere fun a => s3Phase a = 0 ∧ a ≠ label

/-- The phase-flipping one-letter words. -/
noncomputable def flipR : GRegex S3Alphabet :=
  FiniteAlphabet.atomWhere fun a => s3Phase a = 1

/-- All words made only of phase-zero letters, obtained at height zero by
complementing the occurrence of a phase-one letter. -/
noncomputable def neutralWordsR : GRegex S3Alphabet :=
  FiniteAlphabet.onlyWhere fun a => s3Phase a = 0

/-- The PST return block `P = D ∪ C B* C`, with `B*` represented at height
zero. -/
noncomputable def returnBlockR (label : S3Alphabet) : GRegex S3Alphabet :=
  GRegex.union (neutralOtherR label)
    (GRegex.concat (GRegex.concat flipR neutralWordsR) flipR)

@[simp]
theorem denote_labelR (label : S3Alphabet) :
    GRegex.denote (labelR label) = Language.letter label := rfl

@[simp]
theorem denote_labelStarR (label : S3Alphabet) :
...
@[simp]
theorem denote_returnBlockR (label : S3Alphabet) :
    GRegex.denote (returnBlockR label) =
      {word | S3ArrowResidue.SelfLoopReturnBlock s3Phase label word} := by
  classical
  ext word
  ...
  · rintro (⟨d, ⟨hd, hdl⟩, rfl⟩ |
    ...
    · obtain ⟨c₁, hc₁, rfl⟩ := hcword₁
      ...
      exact Or.inr ⟨c₁, middle, c₂, hc₁,
        (by simpa [S3ArrowResidue.IsPhaseZeroWord] using hmiddle), hc₂, by simp⟩
...
@[simp]
theorem starHeight_labelR (label : S3Alphabet) :
    GRegex.starHeight (labelR label) = 0 := rfl

@[simp]
theorem starHeight_labelStarR (label : S3Alphabet) :
    GRegex.starHeight (labelStarR label) = 0 := by
  simp [labelStarR]

@[simp]
theorem starHeight_returnBlockR (label : S3Alphabet) :
    GRegex.starHeight (returnBlockR label) = 0 := by
  simp [returnBlockR, neutralOtherR, flipR, neutralWordsR, GRegex.starHeight]

end S3SelfLoopHeight

/-- A one-letter language is star-free. -/
theorem isStarFree_s3Letter (label : S3Alphabet) :
    IsStarFree (Language.letter label) :=
  ⟨S3SelfLoopHeight.labelR label, S3SelfLoopHeight.denote_labelR label,
    by simp⟩

/-- The star of a singleton letter is star-free in the generalized sense. -/
theorem isStarFree_s3LetterStar (label : S3Alphabet) :
    IsStarFree (Language.star (Language.letter label)) :=
  ⟨S3SelfLoopHeight.labelStarR label,
    S3SelfLoopHeight.denote_labelStarR label, by simp⟩

/-- The concrete two-state return-block language is star-free. -/
theorem isStarFree_s3SelfLoopReturnBlock (label : S3Alphabet) :
    IsStarFree
      {word | S3ArrowResidue.SelfLoopReturnBlock s3Phase label word} :=
  ⟨S3SelfLoopHeight.returnBlockR label,
    S3SelfLoopHeight.denote_returnBlockR label, by simp⟩
