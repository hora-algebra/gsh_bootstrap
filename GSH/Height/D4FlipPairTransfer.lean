...
namespace D4FlipPairTransfer

open ArrowCounting
open S3ArrowArithmetic
open D4ArrowArithmetic
open D4FlipPairHeight
open TransferSemantics

abbrev IsNeutralWord (word : Word D4Alphabet) : Prop :=
  ∀ a ∈ word, d4Phase a = 0

def IsPairBlock (word : Word D4Alphabet) : Prop :=
  ∃ leading x middle y,
    IsNeutralWord leading ∧ d4Phase x = 1 ∧
      IsNeutralWord middle ∧ d4Phase y = 1 ∧
        word = leading ++ x :: middle ++ [y]

def IsPairFactorization (pieces : List (Word D4Alphabet))
    (suffix word : Word D4Alphabet) : Prop :=
  (∀ piece ∈ pieces, IsPairBlock piece) ∧ IsNeutralWord suffix ∧
    pieces.flatten ++ suffix = word

private theorem phase_eq_zero_or_one (p : D4Alphabet) :
    d4Phase p = 0 ∨ d4Phase p = 1 := by
  rcases p with i | i <;> simp [d4Phase]

private theorem run_neutral (start : ZMod 2) (word : Word D4Alphabet)
    (hword : IsNeutralWord word) :
    runFrom (c2Step d4Phase) start word = start := by
  induction word generalizing start with
  | nil => rfl
  | cons a word ih =>
      have ha : d4Phase a = 0 := hword a (by simp)
      have hw : IsNeutralWord word := by
        intro b hb
        exact hword b (by simp [hb])
      simp [runFrom_cons, c2Step, ha, ih start hw]

private theorem exists_first_flip (word : Word D4Alphabet)
    (h : ∃ a ∈ word, d4Phase a = 1) :
    ∃ leading x tail, word = leading ++ x :: tail ∧
      IsNeutralWord leading ∧ d4Phase x = 1 := by
  induction word with
  | nil => simp at h
  | cons a word ih =>
      by_cases ha : d4Phase a = 1
      · exact ⟨[], a, word, rfl, (by intro b hb; simp at hb), ha⟩
      · have ha0 : d4Phase a = 0 :=
          (phase_eq_zero_or_one a).resolve_right ha
        have htail : ∃ b ∈ word, d4Phase b = 1 := by
...
private theorem exists_pairFactorization_of_length :
  ...
  · obtain ⟨leading, x, rest, rfl, hleading, hx⟩ :=
    ...
    have hrunTail : runFrom (c2Step d4Phase) 0 tail = 0 := by
...
theorem exists_pairFactorization (word : Word D4Alphabet)
    (hphase : (word.map d4Phase).sum = 0) :
    ∃ pieces suffix, IsPairFactorization pieces suffix word := by
  have hrun : runFrom (c2Step d4Phase) 0 word = 0 := by
    simpa [c2RunFrom, c2Step] using
      (c2RunFrom_eq d4Phase 0 word).trans (by simp [hphase])
  exact exists_pairFactorization_of_length word.length word rfl hrun

local instance : BEq D4PhaseOne := instBEqOfDecidableEq

theorem phaseOneSubword_append (left right : Word D4Alphabet) :
    d4PhaseOneSubword (left ++ right) =
      d4PhaseOneSubword left ++ d4PhaseOneSubword right := by
  induction left with
  | nil => rfl
  | cons a left ih =>
      by_cases ha : d4Phase a = 1 <;>
        simp [d4PhaseOneSubword, ha, ih]

theorem phaseOneSubword_eq_nil_of_neutral (word : Word D4Alphabet)
    (hword : IsNeutralWord word) : d4PhaseOneSubword word = [] := by
  induction word with
  | nil => rfl
  | cons a word ih =>
      have ha : d4Phase a = 0 := hword a (by simp)
      have htail : IsNeutralWord word := by
        intro b hb
        exact hword b (by simp [hb])
      simp [d4PhaseOneSubword, ha, ih htail]

theorem phaseOneSubword_pairBlock (piece : Word D4Alphabet)
    (hpiece : IsPairBlock piece) :
    ∃ x y : D4PhaseOne, d4PhaseOneSubword piece = [x, y] := by
  rcases hpiece with ⟨leading, x, middle, y, hleading, hx,
    hmiddle, hy, rfl⟩
  refine ⟨⟨x, hx⟩, ⟨y, hy⟩, ?_⟩
  simp [d4PhaseOneSubword, hx, hy, phaseOneSubword_append,
    phaseOneSubword_eq_nil_of_neutral leading hleading,
    phaseOneSubword_eq_nil_of_neutral middle hmiddle]

def pairBlockCount (x y : D4PhaseOne) : List (Word D4Alphabet) → Nat
  | [] => 0
  | piece :: pieces =>
      (if d4PhaseOneSubword piece = [x, y] then 1 else 0) +
        pairBlockCount x y pieces

private theorem pairBlockCount_eq_pairCountOf_flatten
    (x y : D4PhaseOne) (pieces : List (Word D4Alphabet))
    (hpieces : ∀ piece ∈ pieces, IsPairBlock piece) :
    pairBlockCount x y pieces =
      pairCountOf x y (d4PhaseOneSubword pieces.flatten) := by
  induction pieces with
  | nil => simp [pairBlockCount, d4PhaseOneSubword]
...
theorem pairBlockCount_eq_pairCountOf (x y : D4PhaseOne)
    (pieces : List (Word D4Alphabet)) (suffix word : Word D4Alphabet)
    (hfactor : IsPairFactorization pieces suffix word) :
    pairBlockCount x y pieces =
      pairCountOf x y (d4PhaseOneSubword word) := by
  have hsuffix := phaseOneSubword_eq_nil_of_neutral suffix hfactor.2.1
  calc
    pairBlockCount x y pieces =
        pairCountOf x y (d4PhaseOneSubword pieces.flatten) :=
      pairBlockCount_eq_pairCountOf_flatten x y pieces hfactor.1
    _ = pairCountOf x y
        (d4PhaseOneSubword (pieces.flatten ++ suffix)) := by
      rw [phaseOneSubword_append, hsuffix]
      simp
    _ = pairCountOf x y (d4PhaseOneSubword word) := by rw [hfactor.2.2]

private theorem map_neutral (word : Word D4Alphabet)
    (hword : IsNeutralWord word) :
    word.map d4Phase = List.replicate word.length 0 := by
  induction word with
  | nil => rfl
  | cons a word ih =>
      have ha : d4Phase a = 0 := hword a (by simp)
...
theorem pairBlock_eq_of_prefix (left right suffix : Word D4Alphabet)
  ...
  rcases hright with ⟨leading₂, x₂, middle₂, y₂, hl₂, hx₂,
    hm₂, hy₂, rfl⟩
  have hphase := congrArg (List.map d4Phase) hprefix
  have hleadingLength : leading₂.length = leading₁.length := by
    apply zeros_before_one_unique leading₂.length leading₁.length
      ...
      (List.replicate middle₁.length 0 ++ [1] ++ suffix.map d4Phase)
  ...
  have hphaseTail :
      List.replicate middle₂.length 0 ++ [1] =
        List.replicate middle₁.length 0 ++ [1] ++ suffix.map d4Phase := by
    simpa [map_neutral leading₁ hl₁, map_neutral leading₂ hl₂,
      map_neutral middle₁ hm₁, map_neutral middle₂ hm₂,
...
theorem pairFactorization_unique
    (leftPieces rightPieces : List (Word D4Alphabet))
    (leftSuffix rightSuffix word : Word D4Alphabet)
...
def d4PairResidueZero (x y : D4PhaseOne) : Language D4Alphabet :=
  {word | (word.map d4Phase).sum = 0 ∧
    pairCountOf x y (d4PhaseOneSubword word) % 8 = 0}

private theorem mem_pairBlock_iff (x y : D4PhaseOne)
    (word : Word D4Alphabet) :
    word ∈ d4PairBlock x.1 y.1 ↔
      IsPairBlock word ∧ d4PhaseOneSubword word = [x, y] := by
  classical
  constructor
  · intro hword
    ...
    · exact ⟨leading, x.1, middle, y.1, hleading, x.2,
        hmiddle, y.2, by simp⟩
    · simp [d4PhaseOneSubword, x.2, y.2, phaseOneSubword_append,
        phaseOneSubword_eq_nil_of_neutral leading hleading,
        phaseOneSubword_eq_nil_of_neutral middle hmiddle]
  · rintro ⟨hblock, hsubword⟩
    ...
    have hlabels : [⟨a, ha⟩, ⟨b, hb⟩] = [x, y] := by
      simpa [d4PhaseOneSubword, ha, hb, phaseOneSubword_append,
        phaseOneSubword_eq_nil_of_neutral leading hleading,
...
private theorem mem_otherPairBlocks_iff (x y : D4PhaseOne)
    (word : Word D4Alphabet) :
    word ∈ d4OtherPairBlocks x y ↔
      IsPairBlock word ∧ d4PhaseOneSubword word ≠ [x, y] := by
  classical
  simp only [d4OtherPairBlocks, Set.mem_iUnion]
  constructor
  · rintro ⟨pair, hpair⟩
    by_cases hp : pair = (x, y)
    · rw [hp, if_pos rfl] at hpair
      simp at hpair
    · have hmem : word ∈ d4PairBlock pair.1.1 pair.2.1 := by
  ...
  · rintro ⟨hblock, hne⟩
    ...
    have hp : (a, b) ≠ (x, y) := by
      ...
      cases hp
      exact hne hab
    have hmem : word ∈ d4PairBlock a.1 b.1 :=
      (mem_pairBlock_iff a b word).mpr ⟨hblock, hab⟩
    simpa [hp] using hmem

private theorem mem_pairCode_iff (x y : D4PhaseOne)
    (word : Word D4Alphabet) :
    word ∈ d4DistinguishedPairBlock x y ∪ d4OtherPairBlocks x y ↔
      IsPairBlock word := by
  classical
  rw [Set.mem_union]
  simp only [d4DistinguishedPairBlock, mem_pairBlock_iff,
    mem_otherPairBlocks_iff]
  constructor
  · rintro (⟨hblock, -⟩ | ⟨hblock, -⟩) <;> exact hblock
  · intro hblock
    by_cases hsub : d4PhaseOneSubword word = [x, y]
    · exact Or.inl ⟨hblock, hsub⟩
    · exact Or.inr ⟨hblock, hsub⟩

private theorem pairLanguages_disjoint (x y : D4PhaseOne) :
    Disjoint (d4DistinguishedPairBlock x y) (d4OtherPairBlocks x y) := by
  rw [Set.disjoint_left]
  intro word hdist hother
  exact (mem_otherPairBlocks_iff x y word).mp hother |>.2
    ((mem_pairBlock_iff x y word).mp hdist).2

private theorem pairCode_unique (x y : D4PhaseOne) :
    ∀ left right : List (Word D4Alphabet),
      Factorizes (d4DistinguishedPairBlock x y ∪ d4OtherPairBlocks x y)
          left left.flatten →
      Factorizes (d4DistinguishedPairBlock x y ∪ d4OtherPairBlocks x y)
          right right.flatten →
...
private theorem countIn_distinguished_eq_pairBlockCount
    (x y : D4PhaseOne) (pieces : List (Word D4Alphabet))
    (hpieces : ∀ piece ∈ pieces, IsPairBlock piece) :
    countIn (d4DistinguishedPairBlock x y) pieces =
  ...
  | cons piece pieces ih =>
      ...
      have hmem : piece ∈ d4DistinguishedPairBlock x y ↔
          d4PhaseOneSubword piece = [x, y] := by
        simpa [d4DistinguishedPairBlock, hpiece] using
          (mem_pairBlock_iff x y piece)
      have hcountCons :
          countIn (d4DistinguishedPairBlock x y) (piece :: pieces) =
            (if piece ∈ d4DistinguishedPairBlock x y then 1 else 0) +
              countIn (d4DistinguishedPairBlock x y) pieces := by
        unfold countIn
        rw [List.countP_cons]
        by_cases hm : piece ∈ d4DistinguishedPairBlock x y <;>
          simp [hm, Nat.add_comm]
      rw [hcountCons, ih htail]
      by_cases hm : piece ∈ d4DistinguishedPairBlock x y
      · have hs := hmem.mp hm
        simp [pairBlockCount, hm, hs]
      · have hs : d4PhaseOneSubword piece ≠ [x, y] :=
...
private theorem phase_sum_zero_of_pairFactorization
  ...
  apply (d4PhaseOneSubword_even_iff_phase_sum_zero word).mp
  rw [← hfactor.2.2, phaseOneSubword_append,
    phaseOneSubword_eq_nil_of_neutral suffix hfactor.2.1]
  simp only [List.append_nil]
  have heven : ∀ blocks : List (Word D4Alphabet),
      (∀ piece ∈ blocks, IsPairBlock piece) →
        (d4PhaseOneSubword blocks.flatten).length % 2 = 0 := by
...
theorem d4PairResidue_zero_eq_formula (x y : D4PhaseOne) :
    d4PairResidueZero x y =
      Language.concat
        (TransferCore.formula (d4DistinguishedPairBlock x y)
          (d4OtherPairBlocks x y) 8)
        d4Neutral := by
  classical
  ext word
  rw [Language.mem_concat_iff]
  change ((word.map d4Phase).sum = 0 ∧
      pairCountOf x y (d4PhaseOneSubword word) % 8 = 0) ↔ _
  constructor
  · rintro ⟨hphase, hcount⟩
    ...
    rw [mem_formula_partition_iff
      (d4DistinguishedPairBlock x y) (d4OtherPairBlocks x y)
      8 (by omega) (pairLanguages_disjoint x y) (pairCode_unique x y)]
  ...
  · rintro ⟨pre, hpre, suffix, hsuffix, hword⟩
    rw [mem_formula_partition_iff
      (d4DistinguishedPairBlock x y) (d4OtherPairBlocks x y)
      8 (by omega) (pairLanguages_disjoint x y) (pairCode_unique x y)] at hpre
...
theorem hasHeightAtMost_d4PairResidue_zero (x y : D4PhaseOne)
    (hne : x.1 ≠ y.1) :
    HasHeightAtMost (d4PairResidueZero x y) 1 := by
  have hformula := hasHeightAtMost_d4PairFormula x y hne
  exact (hformula.concat (d4Neutral_starFree.mono (by omega))).congr
    (d4PairResidue_zero_eq_formula x y).symm

end D4FlipPairTransfer
